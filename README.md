...
1. The developer account watermarks the PDF with "Developer Demo" in red.  To remove this, you will have to perform this setup with a paid Docusign account, and change the DOCUSIGN_OAUTH_HOST from `account-d.docusign.com` to `account.docusign.com` in `config.gs`, and as well as redo the consent process again with this in the URL. (TODO: move this from docusign.service.gs to config.gs and perhaps create a helper function/page?)
