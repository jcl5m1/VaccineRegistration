...
1. Find your API Username (or UserID) and copy to `var DOCUSIGN_API_USERNAME = 'YOUR_API_USERNAME'` in `config.gs`.  It should look something like `6ddaf671-512f-448b-98bb-c9f123416de5`
3. Once these variables are all defined in `config.gs`, refresh your tool and instead of a spinning wait screen during the registration process, it should redirect you to a docusign flow requesting a signature.  After the signature is complete, if will send the patient a PDF copy of the consent form, upload a PDF copy to your Google Drive, and redirect the patient to the Appointment selection page.
    1. Optional: You can also test if the Docusign keys are working by going to the AppScript project, select `docusign.service.gs`, in the toolbar next to **Debug**, select `testDocusignAPI` from the drop down menu, then hit **Run**.  If all is well, the **Execution Log** should end with *pass*.
5. The developer account watermarks the PDF with "Developer Demo" in red.  To remove this, you will have to perform this setup with a paid Docusign account, and change the `OAUTH_HOST` in `docusign.service.gs` from `account-d.docusign.com` to `account.docusign.com` in `config.gs`, and as well as redo the consent process again with this in the URL. (TODO: move this from `docusign.service.gs` to `config.gs` and perhaps create a helper function/page?)
